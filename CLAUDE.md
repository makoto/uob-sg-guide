...
- **Layer panel** (top-right): collapsible groups with chevron indicators, feature counts per layer, zero-count dimming, provenance filter bar (Gov.sg / Global), source badges (SG / globe icon), RS grid colour mode selector (NDVI, DEM, Slope, LST, NDBI), and Data Catalogue footer button that opens the in-viewer modal.
...
- **LAYERS array**: Each layer entry includes `sourceLabel` (string) and `sourceUrl` (string or null) properties for source attribution and linking, used by the in-viewer Data Catalogue modal.
- **SUBZONE_METRICS constant**: Array of 37 metric definitions with `key`, `label`, `category`, `source`, `aggregate` ('sum' or 'range'), `fmt`, `unit` fields. Used by the Data Catalogue modal to show subzone metric values.
- **In-viewer Data Catalogue modal**: A dark-themed modal overlay opened via the "Data Catalogue" button at the bottom of the layer panel (replaces the previous external link). Built by `buildCatalogueModal()` and populated by `populateCatalogueTable()`. Contains: district-specific title, "Visible layers only" toggle, Map Layers table (6 columns: Vis, Layer, Group, Source, Features, File), Subzone Metrics section (subzone name chips + 4-column table: Metric, Category, Source, Value), footnotes, and a footer link to the static `data-catalogue.html`. The Subzone Metrics section header is followed by a row of inline subzone name chips (`#catalogue-subzone-list`) showing all subzones for the current district sorted alphabetically; chips for subzones excluded by the subzone filter are dimmed (grey text, lighter background). Opened/closed by `openDataCatalogue()` / `closeCatalogue()` (via X button, overlay click, or Escape key). Feature counts and metric values respect the active subzone filter. The modal's open/closed state is persisted in the URL via the `cat=1` parameter, so shared links can include a pre-opened catalogue.
- **Shareable URL state**: The viewer serializes its state into URL query parameters via `history.replaceState` on every state change, enabling shareable links. A "Copy link" button in the info panel (below basemap toggle) copies the current URL to the clipboard. URL parameters (only non-default values are included):
  ...
  - `cat` — `1` to open the Data Catalogue modal on load (omitted when closed)
  Key functions: `serializeState()` builds the query string, `restoreStateFromUrl()` parses URL params on page load and sets state before panel/layer init, `updateUrl()` called on every state change (layer toggle, subzone filter, choropleth, basemap, RS grid mode, district switch, Data Catalogue open/close). `loadDefaultLayers()` loads all URL-specified visible layers (not just hardcoded defaults). `populateSubzoneFilter()` respects pre-set `selectedSubzones` from URL. Choropleth activation is deferred until after default layers finish loading if `choro` param was set. Data Catalogue modal auto-opens after default layers load if `cat=1` was in URL.
